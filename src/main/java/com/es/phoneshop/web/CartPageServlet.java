...
public class CartPageServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        IntStream.range(0, quantities.length).forEach(i -> {
            ...
            try {
                int quantity = parseQuantityFromString(quantities[i], request);
                cartService.update(cart, productId, quantity);
            } catch (OutOfStockException e) {
                errors.put(productId, "Out of stock, available = " + e.getQuantityAvailable());
            }
            catch (IllegalArgumentException e){
                errors.put(productId, "Value must be a positive number");
            }
    ...
    private Integer parseQuantityFromString(String quantity, HttpServletRequest request) throws ParseException {
